...
function fetchDataFromAPI(filters) {
  return axios
...
fetchDataFromAPI(categories).then(categories => {
...
fetchDataFromAPI(area).then(areas => {
...
fetchDataFromAPI(ingredients).then(ingredients =>
...
async function processAllPages(recipes) {
  const response = await fetchDataFromAPI('recipes?page=1');
  ...
  for (let page = 2; page <= totalPages; page += 1) {
    const pageResponse = await fetchDataFromAPI(`recipes?page=${page}`);
